...
> 容量限制：除了源点和汇点，对于每一个点，流入量等于流出量；从源点流出的流量，一定等于最终汇入汇点的流量
...
- 原图中 v-w 容量为 c，流量为 f
- 残量图中 v - w 为 c - f, w - v 为 f
- 增广路径：残量图中每条边权值大于 0 的路径
- *Ford-Fulkerson 思想* : 在残量图中不断找增广路径，直到没有增广路径为止

## Edmonds-Karp

> 使用 BFS 在残量图中寻找增广路径，直到从源点不再能进行 BFS
